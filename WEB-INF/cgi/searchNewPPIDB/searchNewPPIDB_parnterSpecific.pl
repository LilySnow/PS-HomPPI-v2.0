...
sub getIntSeq {
    ...
    if ( !-e $seqFL ) {
        ...
        my ( $seq1, $ATOMindex ) =
          &extractSeqFromPPIDB( $sequencesDIR, $seqFL, $chainID1 ); # $seq1 containts only residues in the PDB file
        my ( $seq2, $ATOMindex2 ) =
          &extractSeqFromPPIDB( $sequencesDIR, $seqFL, $chainID2 ); #$seq2 containts only residues in the PDB file


        #read s2c files
        my ( $s2c_residues, $SEQRESresinum, $ATOMresinum ) =
          &readS2Cfile( $PDBID, $chainID1, $s2cDIR );
        #note: $s2c_residues contains the whole seq (not only the structual residues)
        ...
        if ( scalar @$s2c_residues < length($seq1) ) {
        #s2c seq (ie., the whole seq) is shorter than the ppidb seq (ie, the structual seq) !!

            print
"WARNING: For $PDBID $chainID1, s2c seq (ie. the whole seq) is shorter than the ppidb seq (ie., the structual seq). Probably something wrong with this s2c entry, for example, 1qgc_4. Skip this query: $PDBID $chainID1:$chainID2.\n";
...
#@int is empty. $chainID is not in $PDBID.dis file of PPIDB, because it has no int at all.
            ...
            #
            my $newInt =
              &mapInt2fasta( \%int_atomResiNum, $s2c_residues, $ATOMresinum );
...
sub header_outputFL {
    ...
    print OUTPUT
"#=========================================================================================================\n";
    print OUTPUT "# auto generated file $serverName\n";
...
sub readS2Cfile {
	...
	my @residues; # residues in the whole sequence (not only the structural residues)
	my @SEQRESresinum;
	my @ATOMresinum;
